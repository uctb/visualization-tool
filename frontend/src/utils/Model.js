...
export default class Model {
    constructor() {
        ...
        this.st_raster_gt_train = null;
        this.st_raster_gt = null;
        this.st_raster_pred = null;
        this.graph = [];
        this.temporal_cluster = [];
        this.spatial_cluster = [];
        this.station_info = [];
        this.station_lats = [];
        this.station_lngs = [];
    ...
    update(gt, pred, station_info, graph, cluster) {
        ...
        this.spatial_cluster = cluster[1];
        this.edgeSet = new Set();
        this.time_length = this.st_raster_pred[0].length;
        this.station_num = this.st_raster_pred.length;
        this.invalid_station_index = new Array();
        
        // invalid_station_index: groudTruth is all 0/groudTruth=NaN,Inf
        for (let i = 0; i < this.station_num; i++) {
            let flag = false;
            let meanValue = this.ct.isArrayAllZero(this.st_raster_gt[i]);
        ...
        for (let i = 0; i < this.station_num; i++) {
            for (let j = i + 1; j < this.station_num; j++) {
                if (this.graph[i][j] != 0) {
                    this.edgeSet.add([i, j, this.graph[i][j]]);
                }
            }
        }

        this.station_lats = new Array(this.station_num);
        this.station_lngs = new Array(this.station_num);
        this.error_matrix = new Array(this.time_length);
        this.mae_for_each_station = new Array(this.station_num);
        this.rmse_for_each_station = new Array(this.station_num);
        this.mre_for_each_station = new Array(this.station_num);
        this.mre_for_filter_station = new Array(this.station_num);
        // absolute error 
        this.st_raster_diff = new Array(this.station_num);
        // relative error
        this.st_raster_re = new Array(this.station_num);
        this.st_raster_re_filter = new Array(this.station_num)

        // calculate error matrix, mae, rmse, mre for each station
        for (let i = 0; i < this.station_num; i++) {
            this.station_lats[i] = this.station_info[i][0];
            this.station_lngs[i] = this.station_info[i][1];

            let tmp_mae = 0;
            this.error_matrix[i] = new Array(this.time_length);
        ...
        // Determine whether the number of timestamps is consistent
        ...
        if (this.time_length !== this.ts.length) {
            ...
            console.log("time series is :", this.ts.length)
    ...
    emitMetricsRankList() {
        ...
        document.getElementById('point_rmse').innerText = this.PointRMSE[0] + '/' + this.rmse;
        document.getElementById('point_mae').innerText = this.PointMAE[0] + '/' + this.mae;
        document.getElementById('point_mape').innerText = this.PointMAPE[0] + '%' + '/' + this.mape;
    }
