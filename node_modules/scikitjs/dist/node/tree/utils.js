...
exports.validateY = exports.validateX = void 0;
function validateX(X) {
    ...
    for (let i = 0; i < X.length; i++) {
        let curRow = X[i];
        if (curRow.length === 0) {
            throw new Error(`Rows in X can not be empty, but row ${i} in X is ${curRow}.`);
        }
        for (let j = 0; j < curRow.length; j++) {
            if (typeof curRow[j] !== 'number' || !Number.isFinite(curRow[j])) {
                throw new Error(`X must contain finite non-NaN numbers, but the element at X[${i}][${j}] is ${curRow[j]}`);
            }
        }
    }
}
exports.validateX = validateX;
