...
exports.DecisionTreeRegressor = exports.DecisionTreeClassifier = exports.DecisionTreeBase = exports.DecisionTree = void 0;
const Splitter_1 = require("./Splitter");
const metrics_1 = require("../metrics/metrics");
const Splitter_2 = require("./Splitter");
...
class DecisionTreeBase extends simpleSerializer_1.Serialize {
    splitter;
    stack = [];
    minSamplesLeaf;
    maxDepth;
    ...
    calcMaxFeatures(nFeatures, maxFeatures) {
        ...
        if (typeof maxFeatures === 'number') {
            (0, typesUtils_1.assert)(maxFeatures >= 1, 'maxFeatures must be greater than 1');
    ...
    fit(X, y, samplesSubset) {
        ...
        while (this.stack.length !== 0) {
            ...
            let currentSplit = (0, Splitter_2.makeDefaultSplit)();
...
exports.DecisionTreeBase = DecisionTreeBase;
/**
 ...
 *
 */
class DecisionTreeClassifier extends DecisionTreeBase {
    labelEncoder;
    name;
    constructor({ criterion = 'gini', maxDepth = undefined, minSamplesSplit = 2, minSamplesLeaf = 1, maxFeatures = undefined, minImpurityDecrease = 0.0 } = {}) {
        (0, typesUtils_1.assert)(['gini', 'entropy'].includes(criterion), 'For classification must pass either the "gini" or "entropy" criterion');
        super({
            criterion,
            maxDepth,
            minSamplesSplit,
        ...
        this.labelEncoder = new LabelEncoder_1.LabelEncoder();
        this.name = 'DecisionTreeClassifier';
    }
    fit(X, y) {
        (0, typesUtils_1.assert)((0, typesUtils_1.isScikit1D)(y), 'y value is not a 1D container');
        (0, typesUtils_1.assert)((0, typesUtils_1.isScikit2D)(X), 'X value is not a 2D container');
        let XArray = (0, utils_2.convertScikit2DToArray)(X);
        let yArray = (0, utils_2.convertScikit1DToArray)(y);
        (0, typesUtils_1.assert)(XArray.length === yArray.length, 'X and y must be the same size');
        (0, utils_1.validateX)(XArray); // checks to make sure there are no NaN's etc
        (0, utils_1.validateY)(yArray); // checks to make sure there are no NaN's etc
        let yArrayFixed = this.labelEncoder.fitTransform(yArray);
        super.fit(XArray, (0, utils_2.convertScikit1DToArray)(yArrayFixed));
        return this;
    }
    getNLeaves() {
...
class DecisionTreeRegressor extends DecisionTreeBase {
    name;
    constructor({ criterion = 'squared_error', maxDepth = undefined, minSamplesSplit = 2, minSamplesLeaf = 1, maxFeatures = undefined, minImpurityDecrease = 0.0 } = {}) {
        (0, typesUtils_1.assert)(['squared_error'].includes(criterion), 'Must pass the regression criterion of "squared_error"');
        super({
            criterion,
            maxDepth,
            minSamplesSplit,
...
exports.DecisionTreeRegressor = DecisionTreeRegressor;
