...
const CappedMaxHeap_1 = require("./CappedMaxHeap");
const tf_singleton_1 = require("../tf-singleton");
const child = (parent) => (parent << 1) + 1;
const parent = (child) => (child - 1) >> 1;
/**
 ...
 */
const ceilPow2 = (int32) => {
    (0, typesUtils_1.assert)(0 <= int32 && int32 <= 0x4000_0000, 'ceilPow2(x): x must be in range [0, 1<<30].');
...
class KdTree {
    ...
    static async build({ metric, entries, leafSize = 16 }) {
        (0, typesUtils_1.assert)(1 < leafSize, 'new KdTree({leafSize=16}): leafSize must be a positive number.');
        (0, typesUtils_1.assert)('function' === typeof metric.minDistToBBox, 'new KdTree({metric}): metric must implement `minDistToBBox` function.');
        ...
        const points = Array.from(indices, (_, i) => data.subarray(nFeatures * i, nFeatures * ++i));
        ...
        const randInt = randUtils.randInt((0, randUtils_1.alea)(`KdTree[${nSamples},${nFeatures}]`));
        const swapIndices = (i, j) => {
            const t = indices[i];
            indices[i] = indices[j];
            indices[j] = t;
        };
    ...
    kNearest(k, queryPoints) {
        ...
        (0, typesUtils_1.assert)(_nFeatures === nDim, 'KNeighbors: X_train.shape[1] must equal X_predict.shape[1].');
        // result data
        ...
        if (0 < k && 0 < nQueries) {
            ...
            for (let q = 0; q < nQueries; q++) {
                ...
                const off = k * q;
                const end = k + off;
                heap = new CappedMaxHeap_1.CappedMaxHeap(dists.subarray(off, end), indxs.subarray(off, end));
