...
export class RobustScaler extends TransformerMixin {
    ...
    fit(X) {
        ...
        if (this.withCentering) {
            const quantiles = rowOrientedArray.map((arr) => quantileSeq(removeMissingValuesFromArray(arr), 0.5));
            this.center = this.tf.tensor1d(quantiles);
        }
        if (this.withScaling) {
            const quantiles = rowOrientedArray.map((arr) => quantileSeq(removeMissingValuesFromArray(arr), [
                lowPercentile / 100,
                highPercentile / 100
            ]));
