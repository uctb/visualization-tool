import * as draw from './line_final.js'
import * as Utils from './myfun'
export function drawmap(option){
    ...
    myChart.on('click',function(params){
        ...
        if(FilterInvalidNodeFlag == 0){
            // 过滤无效点开关关闭
            let InitMapoption = createMapOption(data,data['Node']['StationInfo'],StationIndArr,pointID);
            drawmap(InitMapoption);
        }
        else if(FilterInvalidNodeFlag == 1){
            // 过滤无效点开关打开
            let InitMapoption = createMapOption(data,FilterInvalidNodeInfo,InvalidNodeIndArr,pointID);
            drawmap(InitMapoption);
        }

        /*右上折线图*/
        let Newoption = draw.createoption(data,pointID,StartInd,EndInd,MethodID);
        draw.drawline(Newoption, 'container_line');
...
export function createMapOption(obj, StationInfo, StationIndArr, centerID = -1,mapsize = myMapSize)
{
    let option = null;
        ...
        series : [
            {
                ...
                symbolSize: function(number, params) {
                    ...
                    let real_id = Utils.FindRealNodeID(id);
...
export function map(){
    ...
    myChart.on('click', function (params) {
        ...
        /*右上折线图*/
        let Newoption = draw.createoption(data,pointID,StartInd,EndInd,MethodID);
        draw.drawline(Newoption);

        /*error折线图*/
        let RMSE_option = draw.createMetricsoption(MethodID,pointID);
        draw.drawhistogram(RMSE_option);
