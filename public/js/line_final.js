import * as draw from './map.js'
/*create option*/

/*左下：Model Analysis*/
export function createModelAnanlysisOption() {
...
/*右上*/
export function createoption(obj,SelectedNodeID,startIndex = -1,endIndex = -1, methodid){
    ...
    if(RealNodeID != -1) {
            // 遍历所有时间点
        for (let k = 0; k < tmp[methodName]['TrafficNode'].length; k++){     // 真实值和预测值的时间长度不等，预测的段应该是尾端对齐
        ...
        tooltip: {
            ...
            formatter: function (params, ticket, callback) {
                ...
                for(let i = 1; i< params.length;i++)
                {
                    ...
                    if(params[0].data == 0)
                    {
                        if(params[1].data == 0){let error_rate = 0;}
...
export function createBadCaseoption(methodid){
...
export function createTimeBadCaseoption(obj,SelectedNodeID,startIndex = -1,endIndex = -1, methodid){
...
export function createMetricsoption(methodid, pointid){
...
export function createRMSEDistributionoption(methodid, pointMetricRange){
...
/*draw*/
export function drawline(option, id){
...
export function drawhistogram(option, id){
    ...
    myChart.on('click', function (params) {
        ...
        if(FilterInvalidNodeFlag == 0){
            // 过滤无效点开关关闭
            let InitMapoption = draw.createMapOption(data,data['Node']['StationInfo'],StationIndArr,pointID);
            draw.drawmap(InitMapoption);
        }
        else if(FilterInvalidNodeFlag == 1){
            // 过滤无效点开关打开
            let InitMapoption = draw.createMapOption(data,FilterInvalidNodeInfo,InvalidNodeIndArr,pointID);
            draw.drawmap(InitMapoption);
        }
    });
}

export function drawModelAnalysis(option, id){
    let myChart = echarts.init(document.getElementById(id));
    myChart.clear();
    myChart.setOption(option);
    window.addEventListener("resize",function(){
